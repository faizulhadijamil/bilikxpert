...
const {Client, Config, CheckoutAPI, AdyenCheckout} = require('@adyen/api-library');
...
// config.apiKey = 'AQElhmfuXNWTK0Qc+iSSk2Y9q9CeW59vbxshCZshV1EXTAGefxgJhxDBXVsNvuR83LVYjEgiTGAH-R6kuIqL+550D1qTX5kJZCc2qwiPqT3x46lzQOEudnjo=-t3wZv9F8d36xGKqJ';
// config.merchantAccount = 'BabelTestBFIT';
// const client = new Client({ config });
// client.setEnvironment("LIVE", "12ed7539c396525e-BabelTestB");
// const checkout = new CheckoutAPI(client);

class PaymentsAdyenDropIn extends React.Component {
  ...
  state = {
    ...
    paymentSessionData: {
      ...
      // origin: 'https://babelasia-37615.firebaseapp.com',
      // returnUrl: 'https://babelasia-37615.firebaseapp.com',
  ...
  componentDidMount() {
    ...
    // this.props.action.getAdyenConfig();
    ...
    // const client = new Client({ config });
    // client.setEnvironment("LIVE", "12ed7539c396525e-BabelTestB");
    // const checkoutDropIn = new CheckoutAPI(client);
    // const paymentsResponse = checkoutDropIn.paymentMethods({
    //     amount: {
    //         currency: "EUR",
    //         value: 1000,
    //     },
    ...
    // }).then(res => res);
    // const getAdyenPaymentMethodURL = `https://us-central1-babelasia-37615.cloudfunctions.net/getAdyenPaymentMethods`;
    // axios.get(getAdyenPaymentMethodURL)
    // .then(res => {
    //     const adyenPaymentMethodData = res && res.data;
    //     console.log('adyenPaymentMethodData: ', adyenPaymentMethodData);
    //     this.setState({adyenPaymentMethodData});
    // });
    ...
    // this.props.actions.getPaymentMethod((body=>{
    ...
    // // s.src = "https://checkoutshopper-test.adyen.com/checkoutshopper/assets/js/sdk/checkoutSDK.1.9.7.min.js";
    // s.src = "https://checkoutshopper-test.adyen.com/checkoutshopper/sdk/3.7.0/adyen.js";
    // s.integrity = "sha384-aRaL1/W2ASAwZpEWmiHJJAhcc5h+pwDywrocWjla8oCSJaMlY4MC2hOPvAaOcOCq"; // sri hash, where to get?
    // s.origin = "anonymous";
    ...
    // dropinScript.type = 'text/javascript';
    // dropinScript.async = true;
    // dropinScript.innerHTML = "document.write('This is output by document.write()!')";
    // dropinScript.src = "https://checkoutshopper-test.adyen.com/checkoutshopper/sdk/3.6.3/adyen.js";
    // dropinScript.integrity = "sha384-yJSUmaii24aKOI7fvTEvOL4FDDS6V6TaNicpl82vIn5RK1ApcZM94gsejXdCqVfn";
    // dropinScript.crossOrigin = "anonymous";
    
    // var head = document.getElementsByTagName('head').item(0);
    // head.appendChild(dropinScript);
  ...
  //   const configuration = {
  //     // paymentMethodsResponse: this.state.adyenPaymentMethodData, // The `/paymentMethods` response from the server.
  //     paymentMethodsResponse: paymentMethodsResponse, 
  //     // originKey: "http://192.168.2.5:3000/",
  //     originKey: "pub.v2.4115399356091745.aHR0cDovL2xvY2FsaG9zdDo4MDgw.ObWBdJValtqzat-cWl5myr0ecEgc3y36W_sZsjVoWmg",
  //     locale: "en-US",
  //     environment: "test",
  //     onSubmit: (state, dropin) => {
  //       console.log('dropinstate: ', state, dropin);
  ...
  handleAdyenConfiguration = (event) => {
    console.log('handleAdyenConfiguration: ');
    const paymentMethodsResponse = {
      groups:[{ 
        name: "Credit Card",
        type: ["mc", "visa"]
  ...
  //   console.log('theconfiguration: ', configuration);
  ...
  handleSubmit = (event) => {
    ...
    if(invoiceId){
      ...
      // paymentSessionData.returnUrl = `https://babelasia-37615.firebaseapp.com/payments/${invoiceId}`
    }
    if(userId){
  ...
  render() {
    ...
    if (packageId && (invoiceData && invoiceData.get('packageId'))) {
      ...
      vendProductItems.length > 0 && vendProductItems.forEach(vendProductItem=>{
        ...
        if(vendProductItem.vendProductId === '98f9aab1-1c0b-1673-52e5-d9216a84b509'){
          packageDisplayString = '6 Month Term Membership';
          invoiceCurrentPrice = 1500;
        }
        if((vendProductItem.vendProductId === vBuyJan2020Single)||(vendProductItem.vendProductId === vBuyJan2020AllAccess)){
    ...
    // console.log('paymentMethodResponse: ', this.state.adyenPaymentMethodData)
      ...
      <div className={classes.container}>
        ...
        {(invoicePaid && paymentId) &&
          ...
          {(packageDisplayString === 'Monthly Membership' || packageDisplayString === '6 Month Term Membership') &&
            <Typography type="title" component="h1" gutterBottom color="primary">
              {invoiceQuantity} x {'Joining Fee'} - {packageDisplayString === 'Monthly Membership' ? `RM350.00` : `RM100.00`} {hasSST && ` (inclusive of SST)`}
              ...
              <div style={{marginTop:240}}>
                {packageName &&
                  <Typography type="title" component="p" color="primary" style={{marginBottom:8}}>
                    {`Current Plan : ${packageName ? packageName : 'None'} ${packageMonthlyFeeString ? packageMonthlyFeeString : ''}`}
                  </Typography>
                }
                <FormControl>
                  <FormGroup row>
...
PaymentsAdyenDropIn.propTypes = {
  classes: PropTypes.object.isRequired,
};

const PaymentsAdyenDropInStyled = withStyles(styles)(PaymentsAdyenDropIn);
...
export default connect(mapStateToProps, mapDispatchToProps)(PaymentsAdyenDropInStyled)
